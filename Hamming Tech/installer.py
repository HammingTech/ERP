...
import ham_t
from ham_t.defaults import _clear_cache
from ham_t.utils import cint, is_git_url
from ham_t.utils.dashboard import sync_dashboards
...
def _is_scheduler_enabled() -> bool:
	...
	try:
		ham_t.connect()
		enable_scheduler = cint(ham_t.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
		ham_t.db.close()
...
def _new_site(
	...
	"""Install a new ham_t site"""

	from ham_t.utils import get_site_path, scheduler, touch_file
	...
	if not db_name:
		...
		db_name = "_" + hashlib.sha1(os.path.realpath(ham_t.get_site_path()).encode()).hexdigest()[:16]
	...
	apps_to_install = (
		["ham_t"] + (ham_t.conf.get("install_apps") or []) + (list(install_apps) or [])
	)
	...
	scheduler_status = "disabled" if ham_t.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from ham_t.database import setup_database

	if not db_type:
		db_type = ham_t.conf.db_type or "mariadb"
	...
	ham_t.flags.in_install_db = True

	ham_t.flags.root_login = root_login
	ham_t.flags.root_password = root_password
	setup_database(force, source_sql, verbose, no_mariadb_socket)

	ham_t.conf.admin_password = ham_t.conf.admin_password or admin_password

	remove_missing_apps()

	ham_t.db.create_auth_table()
	ham_t.db.create_global_search_table()
	ham_t.db.create_user_settings_table()

	ham_t.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/ham_t/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from ham_t.exceptions import InvalidRemoteException

	for org in ["ham_t", "erpnext"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/ham_t/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/ham_t/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	from ham_t.core.doctype.scheduled_job_type.scheduled_job_type import sync_jobs
	from ham_t.model.sync import sync_for
	from ham_t.modules.utils import sync_customizations
	from ham_t.utils.fixtures import sync_fixtures

	ham_t.flags.in_install = name
	ham_t.flags.ignore_in_install = False

	ham_t.clear_cache()
	app_hooks = ham_t.get_hooks(app_name=name)
	installed_apps = ham_t.get_installed_apps()
	...
	if name not in ham_t.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = ham_t.get_attr(before_install)()
		if out is False:
			return
	...
	ham_t.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		ham_t.get_attr(after_install)()
	...
	for after_sync in app_hooks.after_sync or []:
		ham_t.get_attr(after_sync)()  #

	ham_t.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		ham_t.db.set_global("installed_apps", json.dumps(installed_apps))
		ham_t.db.commit()
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	site = ham_t.local.site
	app_hooks = ham_t.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in ham_t.get_installed_apps():
	...
	if not (dry_run or no_backup):
		from ham_t.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	ham_t.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		ham_t.get_attr(before_uninstall)()

	modules = ham_t.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	if not dry_run:
		...
		ham_t.get_single("Installed Applications").update_versions()
		ham_t.db.commit()

	for after_uninstall in app_hooks.after_uninstall or []:
		ham_t.get_attr(after_uninstall)()

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	ham_t.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in ham_t.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					ham_t.delete_doc("DocType", doctype.name, ignore_on_trash=True)
				else:
		...
		if not dry_run:
			ham_t.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in ham_t.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				ham_t.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = ham_t.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if ham_t.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			ham_t.delete_doc("DocType", doctype, ignore_on_trash=True)
			ham_t.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from ham_t.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		ham_t.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	ham_t.db.commit()


def init_singles():
	singles = ham_t.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if ham_t.db.get_singles_dict(single):
			continue

		try:
			doc = ham_t.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, ham_t.DoesNotExistError):
...
def make_conf(
	...
	sites_path = ham_t.local.sites_path
	ham_t.destroy()
	ham_t.init(site, sites_path=sites_path)


def make_site_config(
	db_name=None, db_password=None, site_config=None, db_type=None, db_host=None, db_port=None
):
	ham_t.create_folder(os.path.join(ham_t.local.site_path))
...
def update_site_config(key, value, validate=True, site_config_path=None):
	...
	if hasattr(ham_t.local, "conf"):
		ham_t.local.conf[key] = value


def get_site_config_path():
	return os.path.join(ham_t.local.site_path, "site_config.json")
...
def get_conf_params(db_name=None, db_password=None):
	...
	if not db_password:
		from ham_t.utils import random_string
...
def make_site_dirs():
	for dir_path in [
		...
		path = ham_t.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = ham_t.get_module_list(app)
	for module in modules:
		d = ham_t.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("ham_t_subscription", "shopping_cart")
	installed_apps = json.loads(ham_t.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_sql_from_archive(sql_file_path):
	...
	"""
	from ham_t.utils import get_bench_relative_path
...
def extract_files(site_name, file_path):
	...
	# Need to do ham_t.init to maintain the site locals
	ham_t.init(site=site_name)
	abs_site_path = os.path.abspath(ham_t.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	if ham_t.conf.db_type not in (None, "mariadb"):
		return False
	...
	with open(sql_file_path) as f:
		for line in f:
			if head in line:
				...
				# 'all_apps' (list) format: [('ham_t', '12.x.x-develop ()', 'develop'), ('your_custom_app', '0.0.1', 'master')]
				...
				for app in all_apps:
					...
					if app_name == "ham_t":
						try:
							current_version = Version(ham_t.__version__)
						...
						if verbose and downgrade:
							print(f"Your site will be downgraded from ham_t {backup_version} to {current_version}")
...
def partial_restore(sql_file_path, verbose=False):
	...
	if ham_t.conf.db_type in (None, "mariadb"):
		from ham_t.database.mariadb.setup_db import import_db_from_sql
	elif ham_t.conf.db_type == "postgres":
		import warnings

		from ham_t.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path, _raise=True):
	...
	if _raise and (missing_table or empty_file):
		raise ham_t.InvalidDatabaseFile
