...
import ham_t
from ham_t import _, _dict
from ham_t.model import (
	child_table_fields,
	datetime_fields,
	default_fields,
	display_fieldtypes,
...
from ham_t.model.naming import set_new_name
from ham_t.model.utils.link_count import notify_link_count
from ham_t.modules import load_doctype_module
from ham_t.utils import cast_fieldtype, cint, cstr, flt, now, sanitize_html, strip_html
from ham_t.utils.html_utils import unescape_html
...
def get_controller(doctype):
	...
	For `custom` type, returns `ham_t.model.document.Document`.

	:param doctype: DocType name as string."""

	def _get_controller():
		from ham_t.model.document import Document
		from ham_t.utils.nestedset import NestedSet

		module_name, custom = ham_t.db.get_value(
			"DocType", doctype, ("module", "custom"), cache=True
		) or ("Core", False)

		if custom:
			is_tree = ham_t.db.get_value("DocType", doctype, "is_tree", ignore=True, cache=True)
			_class = NestedSet if is_tree else Document
		else:
			class_overrides = ham_t.get_hooks("override_doctype_class")
			if class_overrides and class_overrides.get(doctype):
				import_path = class_overrides[doctype][-1]
				module_path, classname = import_path.rsplit(".", 1)
				module = ham_t.get_module(module_path)
	...
	if ham_t.local.dev_server:
		return _get_controller()

	site_controllers = ham_t.controllers.setdefault(ham_t.local.site, {})
...
class BaseDocument:
	...
	def meta(self):
		if not (meta := getattr(self, "_meta", None)):
			self._meta = meta = ham_t.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return ham_t.db.get_value(self.doctype, self.name, key)
			...
			if df:
				if getattr(df, "is_virtual", False):
					...
					if d[fieldname] is None and (options := getattr(df, "options", None)):
						from ham_t.utils.safe_exec import get_safe_globals

						d[fieldname] = ham_t.safe_eval(
							code=options,
				...
				if isinstance(d[fieldname], list) and df.fieldtype not in table_fields:
					ham_t.throw(_("Value for {0} cannot be a list").format(_(df.label)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in ham_t.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from ham_t.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			ham_t.local.valid_columns[self.doctype] = valid

		return ham_t.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return ham_t.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and ham_t.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.created_by = self.modified_by = ham_t.session.user
		...
		try:
			...
			if ham_t.db.is_primary_key_violation(e):
				if self.meta.autoname == "hash":
					# hash collision? try again
					ham_t.flags.retry_count = (ham_t.flags.retry_count or 0) + 1
					if ham_t.flags.retry_count > 5 and not ham_t.flags.in_test:
						raise
					self.name = None
					self.db_insert()
					return
				...
				if not ignore_if_duplicate:
					ham_t.msgprint(
						_("{0} {1} already exists").format(self.doctype, ham_t.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise ham_t.DuplicateEntryError(self.doctype, self.name, e)

			elif ham_t.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if ham_t.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if ham_t.db.db_type != "postgres":
			...
			ham_t.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise ham_t.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		ham_t.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get(
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						ham_t.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						ham_t.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not ham_t.get_meta(doctype).get("is_virtual"):
					if not fields_to_fetch:
						# cache a single value type
						values = _dict(name=ham_t.db.get_value(doctype, docname, "name", cache=True))
					else:
						values_to_fetch = ["name"] + [_df.fetch_from.split(".")[-1] for _df in fields_to_fetch]

						# don't cache if fetching other values too
						values = ham_t.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if getattr(ham_t.get_meta(doctype), "issingle", 0):
					values.name = doctype

				if ham_t.get_meta(doctype).get("is_virtual"):
					values = ham_t.get_doc(doctype, docname)
				...
				if values:
					...
					if not values.name:
						...
						and ham_t.get_meta(doctype).is_submittable
						and cint(ham_t.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from ham_t.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or ham_t.get_meta(doctype).get_field(
				fetch_from_fieldname
			)

			if not fetch_from_df:
				ham_t.throw(
					_('Please check the value of "Fetch From" set for field {0}').format(ham_t.bold(df.label)),
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (ham_t.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		# data_field options defined in ham_t.model.data_field_options
		for phone_field in self.meta.get_phone_fields():
			phone = self.get(phone_field.fieldname)
			ham_t.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in ham_t.STANDARD_USERS) and (data in ham_t.STANDARD_USERS):
					continue
				for email_address in ham_t.utils.split_emails(data):
					ham_t.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				ham_t.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				ham_t.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				ham_t.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if ham_t.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = ham_t.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				ham_t.throw(
					...
					ham_t.CannotChangeConstantError,
				)

	def _validate_length(self):
		if ham_t.flags.in_install:
			return
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				ham_t.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				ham_t.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		ham_t.throw(
			...
			ham_t.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = ham_t.get_doc(self.doctype, self.name).as_dict()
	...
	def _save_passwords(self):
		...
		from ham_t.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_password(self, fieldname="password", raise_exception=True):
		from ham_t.utils.password import get_decrypted_password
	...
	def precision(self, fieldname, parentfield=None):
		...
		from ham_t.model.meta import get_field_precision
	...
	def get_formatted(
		...
		from ham_t.utils.formatters import format_value
		...
		if (
			...
			currency = ham_t.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = ham_t.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from ham_t.core.doctype.file.utils import extract_images_from_doc
...
def _filter(data, filters, limit=None):
	...
	for d in data:
		for f, fval in _filters.items():
			if not ham_t.compare(getattr(d, f, None), fval[0], fval[1]):
				break
		else:
			out.append(d)
