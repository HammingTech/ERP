...
from ham_t.model.base_document import (
	DOCTYPE_TABLE_FIELDS,
	TABLE_DOCTYPES_FOR_DOCTYPE,
	BaseDocument,
)
from ham_t.model.document import Document
from ham_t.model.workflow import get_workflow_name
from ham_t.modules import load_doctype_module
from ham_t.utils import cast, cint, cstr
...
def get_meta(doctype, cached=True) -> "Meta":
	...
	if meta := ham_t.cache().hget("doctype_meta", doctype):
		return meta

	meta = Meta(doctype)
	ham_t.cache().hset("doctype_meta", doctype, meta)
	return meta
...
def get_table_columns(doctype):
	return ham_t.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = ham_t.scrub(doctype)
	with open(ham_t.get_app_path("ham_t", "core", "doctype", fname, fname + ".json")) as f:
...
class Meta(Document):
	...
	standard_set_once_fields = [
		ham_t._dict(fieldname="creation", fieldtype="Datetime"),
		ham_t._dict(fieldname="owner", fieldtype="Data"),
	]
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(ham_t._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self):
		if not hasattr(self, "_valid_columns"):
			table_exists = ham_t.db.table_exists(self.name)
	...
	def add_custom_fields(self):
		if not ham_t.db.table_exists("Custom Field"):
			return

		custom_fields = ham_t.db.sql(
			"""
	...
	def apply_property_setters(self):
		...
		"""
		if not ham_t.db.table_exists("Property Setter"):
			return

		property_setters = ham_t.db.sql(
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(ham_t._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = ham_t.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other ham_t applications via hooks.
		"""
		data = ham_t._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = ham_t._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in ham_t.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = ham_t._dict(ham_t.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = ham_t.scrub(self.module)
		doctype = ham_t.scrub(self.name)
		template_path = ham_t.get_module_path(
...
def is_single(doctype):
	try:
		return ham_t.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	if not ham_t.is_table(dt):
		return ""

	return (
		ham_t.db.get_value(
			"DocField",
			{"fieldtype": ("in", ham_t.model.table_fields), "options": dt},
			"parent",
		)
		or ""
	)
...
def set_fieldname(field_id, fieldname):
	ham_t.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(ham_t.local, "field_currency", None):
		ham_t.local.field_currency = ham_t._dict()

	if not (
		ham_t.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and ham_t.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = ham_t.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if ham_t.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = ham_t.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			ham_t.local.field_currency.setdefault((doc.doctype, ref_docname), ham_t._dict()).setdefault(
				df.fieldname, currency
			)

	return ham_t.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent")
		and ham_t.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)
...
def get_field_precision(df, doc=None, currency=None):
	...
	from ham_t.utils import get_number_format_info

	if df.precision:
		precision = cint(df.precision)

	elif df.fieldtype == "Currency":
		precision = cint(ham_t.db.get_default("currency_precision"))
		if not precision:
			number_format = ham_t.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(ham_t.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return ham_t._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return ham_t._dict(fieldname=fieldname, fieldtype="Int")

		return ham_t._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in ham_t.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	ham_t.cache().hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = ham_t.db.get_table_columns(doctype)
	fields = ham_t.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		ham_t.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
