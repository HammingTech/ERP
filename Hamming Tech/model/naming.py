...
import ham_t
from ham_t import _
from ham_t.model import log_types
from ham_t.query_builder import DocType
from ham_t.utils import cint, cstr, now_datetime
...
class InvalidNamingSeriesError(ham_t.ValidationError):
	pass
...
class NamingSeries:
	...
	def validate(self):
		if "." not in self.series:
			ham_t.throw(
				_("Invalid naming series {}: dot (.) missing").format(ham_t.bold(self.series)),
	...
	def get_prefix(self) -> str:
		...
		if prefix is None:
			ham_t.throw(_("Invalid Naming Series: {}").format(self.series))
	...
	def update_counter(self, new_count: int) -> None:
		...
		Series = ham_t.qb.DocType("Series")
		prefix = self.get_prefix()

		# Initialize if not present in DB
		if ham_t.db.get_value("Series", prefix, "name", order_by="name") is None:
			ham_t.qb.into(Series).insert(prefix, 0).columns("name", "current").run()

		(
			ham_t.qb.update(Series).set(Series.current, cint(new_count)).where(Series.name == prefix)
		).run()

	def get_current_value(self) -> int:
		prefix = self.get_prefix()
		return cint(ham_t.db.get_value("Series", prefix, "current", order_by="name"))
...
def set_new_name(doc):
	...
	meta = ham_t.get_meta(doc.doctype)
	autoname = meta.autoname or ""

	if autoname.lower() != "prompt" and not ham_t.flags.in_import:
		doc.name = None

	if is_autoincremented(doc.doctype, meta):
		doc.name = ham_t.db.get_next_sequence_val(doc.doctype)
		return
...
def is_autoincremented(doctype: str, meta: Optional["Meta"] = None) -> bool:
	...
	if doctype in log_types:
		if autoincremented_site_status_map.get(ham_t.local.site) is None:
			if (
				ham_t.db.sql(
				...
				autoincremented_site_status_map[ham_t.local.site] = 1
				return True
			else:
				autoincremented_site_status_map[ham_t.local.site] = 0

		elif autoincremented_site_status_map[ham_t.local.site]:
			return True
...
def set_name_from_naming_options(autoname, doc):
	...
	if _autoname.startswith("field:"):
		...
		if not doc.name:
			...
			ham_t.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def set_naming_from_document_naming_rule(doc):
	...
	for d in ham_t.get_all(
		...
		ham_t.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		ham_t.throw(ham_t._("Naming Series mandatory"))
...
def make_autoname(key="", doctype="", doc=""):
	...
	*Example:*
	...
	"""
	if key == "hash":
		return ham_t.generate_hash(doctype, 10)
...
def parse_naming_series(
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = ham_t.defaults.get_user_default("fiscal_year")
...
def getseries(key, digits):
	# series created ?
	# Using ham_t.qb as ham_t.get_values does not allow order_by=None
	series = DocType("Series")
	current = (ham_t.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		ham_t.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		ham_t.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (
		ham_t.qb.from_(series).where(series.name == prefix).for_update().select("current")
	).run()

	if current and current[0][0] == count:
		ham_t.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype: str) -> str | None:
	"""get default value for `naming_series` property"""
	naming_series_options = ham_t.get_meta(doctype).get_naming_series_options()
...
def validate_name(doctype: str, name: int | str, case: str | None = None):
	...
	if not name:
		ham_t.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		if is_autoincremented(doctype):
			...
			ham_t.db.set_next_sequence_val(doctype, name, is_val_used=True)
			return name

		ham_t.throw(_("Invalid name type (integer) for varchar name column"), ham_t.NameError)

	if name.startswith("New " + doctype):
		ham_t.throw(
			_("There were some errors setting the name, please contact the administrator"), ham_t.NameError
		)
	if case == "Title Case":
		name = name.title()
	if case == "UPPER CASE":
	...
	if not ham_t.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		ham_t.throw(_("Name of {0} cannot be {1}").format(doctype, name), ham_t.NameError)

	special_characters = "<>"
	if re.findall(f"[{special_characters}]+", name):
		message = ", ".join(f"'{c}'" for c in special_characters)
		ham_t.throw(
			_("Name cannot contain special characters like {0}").format(message), ham_t.NameError
		)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = ham_t.db.exists(doctype, filters)
	...
	if exists:
		last = ham_t.db.sql(
			...
			`{fieldname}` DESC LIMIT 1""".format(
				doctype=doctype, fieldname=fieldname, regex_character=ham_t.db.REGEX_CHARACTER
			),
			regex,
		)
...
def _set_amended_name(doc):
	...
	if ham_t.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		ham_t.throw(_("Please set the document name"))
