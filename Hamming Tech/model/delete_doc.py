...
from ham_t import _, get_module_path
from ham_t.desk.doctype.tag.tag import delete_tags_for_document
from ham_t.model.dynamic_links import get_dynamic_link_map
from ham_t.model.naming import revert_series_if_last
from ham_t.model.utils import is_virtual_doctype
from ham_t.utils.file_manager import remove_all
from ham_t.utils.global_search import delete_for_document
from ham_t.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = ham_t.form_dict.get("dt")
		name = ham_t.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			ham_t.get_doc(doctype, name).delete()
			continue

		# already deleted..?
		if not ham_t.db.exists(doctype, name):
		...
		if doctype == "DocType":
			if for_reload:
				...
				ham_t.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				ham_t.conf.developer_mode
				and not doc.custom
				and not (
					for_reload or ham_t.flags.in_migrate or ham_t.flags.in_install or ham_t.flags.in_uninstall
				)
			):
				try:
			...
			if not for_reload:
				...
				ham_t.enqueue(
					"ham_t.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
					now=ham_t.flags.in_test,
				)
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and ham_t.flags.in_install != "ham_t":
		ham_t.get_doc(
			dict(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		ham_t.db.delete("Singles", {"doctype": name})
	else:
		ham_t.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if ham_t.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = ham_t.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", ham_t.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		ham_t.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and ham_t.session.user != "Administrator"
		...
		ham_t.msgprint(
			...
			raise_exception=ham_t.PermissionError,
		)
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from ham_t.model.rename_doc import get_link_fields
	...
	for lf in link_fields:
		...
		if not issingle:
			...
			if ham_t.get_meta(link_dt).istable:
				fields.extend(["parent", "parenttype"])

			for item in ham_t.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
			...
			if ham_t.db.get_value(link_dt, None, link_field) == doc.name:
				raise_link_exists_exception(doc, link_dt, link_dt)


def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `ham_t.LinkExistsError` if the document is dynamically linked"""
...
def clear_timeline_references(link_doctype, link_name):
	ham_t.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})


def insert_feed(doc):
	if (
		ham_t.flags.in_install
		or ham_t.flags.in_uninstall
		or ham_t.flags.in_import
		or getattr(doc, "no_feed_on_delete", False)
	):
		return

	from ham_t.utils import get_fullname
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", ham_t.scrub(doctype))
