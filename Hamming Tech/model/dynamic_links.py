...
def get_dynamic_link_map(for_delete=False):
	...
	"""
	if getattr(ham_t.local, "dynamic_link_map", None) is None or ham_t.flags.in_test:
		# Build from scratch
		dynamic_link_map = {}
		for df in get_dynamic_links():
			...
			if meta.issingle:
				...
				try:
					links = ham_t.db.sql_list("""select distinct {options} from `tab{parent}`""".format(**df))
					for doctype in links:
						dynamic_link_map.setdefault(doctype, []).append(df)
				except ham_t.db.TableMissingError:  # noqa: E722
					pass

		ham_t.local.dynamic_link_map = dynamic_link_map
	return ham_t.local.dynamic_link_map
...
def get_dynamic_links():
	...
	for query in dynamic_link_queries:
		df += ham_t.db.sql(query, as_dict=True)
	return df
