...
import ham_t
from ham_t import _
from ham_t.model import child_table_fields, default_fields, table_fields
from ham_t.utils import cstr
...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	for hook in ham_t.get_hooks("override_whitelisted_methods", {}).get(method, []):
	...
	if method not in ham_t.whitelisted:
	...
	if args:
		ham_t.flags.args = ham_t._dict(json.loads(args))

	ham_t.flags.selected_children = selected_children or None
...
def get_mapped_doc(
	...
	apply_strict_user_permissions = ham_t.get_system_settings("apply_strict_user_permissions")

	# main
	if not target_doc:
		target_doc = ham_t.new_doc(table_maps[from_doctype]["doctype"])
	elif isinstance(target_doc, str):
		target_doc = ham_t.get_doc(json.loads(target_doc))
	...
	source_doc = ham_t.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				for source_d in source_doc.get(df.fieldname):
					...
					if (
						ham_t.flags.selected_children
						and (df.fieldname in ham_t.flags.selected_children)
						and source_d.name not in ham_t.flags.selected_children[df.fieldname]
					):
						continue
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": f"^{df.fieldname}."}):
		...
		if (
			...
			if not linked_doc:
				try:
					linked_doc = ham_t.get_doc(df.options, target_doc.get(df.fieldname))
				except Exception:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = ham_t.new_doc(target_child_doctype, target_parent, target_parentfield)
