...
from ham_t.database.utils import FallBackDateTimeStr
from ham_t.model import optional_fields
from ham_t.model.meta import get_table_columns
from ham_t.model.utils.user_settings import get_user_settings, update_user_settings
from ham_t.query_builder.utils import Column
from ham_t.utils import (
	add_to_date,
	cint,
	cstr,
	flt,
...
class DatabaseQuery:
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or ham_t.session.user
	...
	def execute(
		...
		*,
		...
		if (
			...
			and not ham_t.has_permission(self.doctype, "select", user=user, parent_doctype=parent_doctype)
			and not ham_t.has_permission(self.doctype, "read", user=user, parent_doctype=parent_doctype)
		):
			ham_t.flags.error_message = _("Insufficient Permission for {0}").format(
				ham_t.bold(self.doctype)
			)
			raise ham_t.PermissionError(self.doctype)
	...
	def build_and_run(self):
		...
		if ham_t.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def prepare_args(self):
		...
		for child in self.tables[1:]:
			...
			args.tables += f" {self.join} {child} on ({child}.parenttype = {ham_t.db.escape(self.doctype)} and {child}.parent = {parent_name})"
	...
	def parse_args(self):
		...
		for field in self.fields:
			if "." in field and "tab" not in field:
				...
				linked_field = ham_t.get_meta(self.doctype).get_field(linked_fieldname)
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			ham_t.throw(_("Use of sub-query or function is restricted"), ham_t.DataError)
		...
		for field in self.fields:
			...
			if self.strict:
				if STRICT_FIELD_PATTERN.match(field):
					ham_t.throw(_("Illegal SQL Query"))
	...
	def append_link_table(self, doctype, fieldname):
		...
		self.link_tables.append(
			ham_t._dict(doctype=doctype, fieldname=fieldname, table_name=f"`tab{doctype}`")
		)

	def check_read_permission(self, doctype):
		ptype = "select" if ham_t.only_has_select_perm(doctype) else "read"

		if not self.flags.ignore_permissions and not ham_t.has_permission(
			doctype, ptype=ptype, parent_doctype=self.doctype
		):
			ham_t.flags.error_message = _("Insufficient Permission for {0}").format(ham_t.bold(doctype))
			raise ham_t.PermissionError(doctype)
	...
	def prepare_filter_condition(self, f):
		...
		from ham_t.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in (
			...
			lft, rgt = "", ""
			if f.value:
				lft, rgt = ham_t.db.get_value(ref_doctype, f.value, ["lft", "rgt"])

			# Get descendants elements of a DocType with a tree structure
			if f.operator.lower() in ("descendants of", "not descendants of"):
				result = ham_t.get_all(
			...
			fallback = "''"
			value = [ham_t.db.escape((cstr(v.name) or "").strip(), percent=False) for v in result]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			fallback = "''"
			value = [ham_t.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			if f.operator in (">", "<", ">=", "<=") and (f.fieldname in ("creation", "modified")):
				...
				value = ham_t.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype == "Datetime") or isinstance(f.value, datetime):
				value = ham_t.db.format_datetime(f.value)
				fallback = f"'{FallBackDateTimeStr}'"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if ham_t.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif isinstance(value, str) and f.operator.lower() != "between":
				value = f"{ham_t.db.escape(value, percent=False)}"
		...
		if (
			...
			if f.operator.lower() == "like" and ham_t.conf.get("db_type") == "postgres":
				f.operator = "ilike"
	...
	def build_match_conditions(self, as_condition=True) -> str | list:
		...
		if not self.user:
			self.user = ham_t.session.user

		if not self.tables:
			self.extract_tables()

		meta = ham_t.get_meta(self.doctype)
		role_permissions = ham_t.permissions.get_role_permissions(meta, user=self.user)
		self.shared = ham_t.share.get_shared(self.doctype, self.user)
		...
		if (
			...
			only_if_shared = True
			if not self.shared:
				ham_t.throw(_("No permission to read {0}").format(_(self.doctype)), ham_t.PermissionError)
			else:
			...
			if requires_owner_constraint(role_permissions):
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {ham_t.db.escape(self.user, percent=False)}"
				)

			# add user permission only if role has read perm
			elif role_permissions.get("read") or role_permissions.get("select"):
				# get user permissions
				user_permissions = ham_t.permissions.get_user_permissions(self.user)
	...
	def get_share_condition(self):
		return (
			...
			+ f" in ({', '.join(ham_t.db.escape(s, percent=False) for s in self.shared)})"
		)
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if ham_t.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					values = ", ".join(ham_t.db.escape(doc, percent=False) for doc in docs)
	...
	def get_permission_query_conditions(self):
		conditions = []
		condition_methods = ham_t.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			for method in condition_methods:
				c = ham_t.call(ham_t.get_attr(method), self.user)
				if c:
					conditions.append(c)

		permision_script_name = get_server_script_map().get("permission_query", {}).get(self.doctype)
		if permision_script_name:
			script = ham_t.get_doc("Server Script", permision_script_name)
	...
	def validate_order_by_and_group_by(self, parameters):
		...
		if "select" in _lower and "from" in _lower:
			ham_t.throw(_("Cannot use sub-query in order by"))
		...
		for field in parameters.split(","):
			if "." in field and field.strip().startswith("`tab"):
				...
				if tbl not in self.tables:
					if tbl.startswith("`"):
						tbl = tbl[4:-1]
					ham_t.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			ham_t.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or ham_t.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
			raise ham_t.PermissionError

		if ham_t.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = ham_t.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	"""
	from_date = ham_t.utils.nowdate()
	to_date = ham_t.utils.nowdate()
	...
	if df and df.fieldtype == "Datetime":
		data = "'{}' AND '{}'".format(
			ham_t.db.format_datetime(from_date),
			ham_t.db.format_datetime(to_date),
		)
	else:
		data = f"'{ham_t.db.format_date(from_date)}' AND '{ham_t.db.format_date(to_date)}'"
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = ham_t._dict(ham_t.get_attr(additional_filter["get_field"])())
	if f.query_value:
		for option in f.options:
			option = ham_t._dict(option)
