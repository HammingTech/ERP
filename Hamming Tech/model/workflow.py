...
class WorkflowStateError(ham_t.ValidationError):
	pass


class WorkflowTransitionError(ham_t.ValidationError):
	pass


class WorkflowPermissionError(ham_t.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = ham_t.cache().hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = ham_t.db.get_value(
			"Workflow", {"document_type": doctype, "is_active": 1}, "name"
		)
		ham_t.cache().hset("workflow", doctype, workflow_name or "")
...
def get_transitions(doc, workflow=None, raise_exception=False):
	...
	doc = ham_t.get_doc(ham_t.parse_json(doc))
	...
	ham_t.has_permission(doc, "read", throw=True)
	...
	if not current_state:
		if raise_exception:
			...
			ham_t.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to ham_t.db.get_value, ham_t.db.get_list, and date time utils.
	return dict(
		ham_t=ham_t._dict(
			db=ham_t._dict(get_value=ham_t.db.get_value, get_list=ham_t.db.get_list),
			session=ham_t.session,
			utils=ham_t._dict(
				now_datetime=ham_t.utils.now_datetime,
				add_to_date=ham_t.utils.add_to_date,
				get_datetime=ham_t.utils.get_datetime,
				now=ham_t.utils.now,
			),
		)
	)
...
def apply_workflow(doc, action):
	...
	if not transition:
		ham_t.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		ham_t.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus == DocStatus.draft():
		...
		ham_t.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if current_state != next_state:
		bold_current = ham_t.bold(current_state)
		bold_next = ham_t.bold(next_state)
...
def get_workflow(doctype):
	return ham_t.get_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	value = ham_t.cache().hget("workflow_" + workflow_name, field)
	if value is None:
		value = ham_t.db.get_value("Workflow", workflow_name, field)
		ham_t.cache().hset("workflow_" + workflow_name, field, value)
	return value
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	print("Clearing ham_t.message_log...")
	ham_t.clear_messages()
	...
	for (idx, docname) in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(ham_t.get_doc(doctype, docname), action)
			ham_t.db.commit()
		except Exception as e:
			if not ham_t.message_log:
			...
			if not message_dict:
				if ham_t.message_log:
					messages = ham_t.get_message_log()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{ham_t.utils.get_link_to_form(doctype, doc)}</summary>"
		...
		ham_t.msgprint(msg, title=_("Workflow Status"), indicator=indicator, is_minimizable=True)
	...
	try:
		for (i, doc) in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(ham_t.session.user, doc, t)
			]
			if not actions:
				return []
...
def show_progress(docnames, message, i, description):
	n = len(docnames)
	if n >= 5:
		ham_t.publish_progress(float(i) * 100 / n, title=message, description=description)


def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = ham_t.get_doc("Workflow", workflow_name)
