...
import ham_t
from ham_t import _, is_whitelisted, msgprint
from ham_t.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from ham_t.desk.form.document_follow import follow_document
from ham_t.integrations.doctype.webhook import run_webhooks
from ham_t.model import optional_fields, table_fields
from ham_t.model.base_document import BaseDocument, get_controller
from ham_t.model.docstatus import DocStatus
from ham_t.model.naming import set_new_name, validate_name
from ham_t.model.utils import is_virtual_doctype
from ham_t.model.workflow import set_workflow_state_on_action, validate_workflow
from ham_t.utils import cstr, date_diff, file_lock, flt, get_datetime_str, now
from ham_t.utils.data import get_absolute_url
from ham_t.utils.global_search import update_global_search


def get_doc(*args, **kwargs):
	"""returns a ham_t.model.Document object.
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = ham_t.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = ham_t.new_doc(self.doctype, as_dict=True)
			...
			d = ham_t.db.get_value(
				self.doctype, self.name, "*", as_dict=1, for_update=self.flags.for_update
			)
			if not d:
				ham_t.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), ham_t.DoesNotExistError
				)
		...
		for df in self._get_table_fields():
			...
			# During ham_t is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to ham_t.db.get_value() as it would crash otherwise.
	...
	def get_latest(self):
		if not getattr(self, "latest", None):
			self.latest = ham_t.get_doc(self.doctype, self.name)
		return self.latest

	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `ham_t.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permlevel or permtype)

	def has_permission(self, permtype="read", verbose=False) -> bool:
		"""
		Call `ham_t.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		import ham_t.permissions

		return ham_t.permissions.has_permission(self.doctype, permtype, self)

	def raise_no_permission_to(self, perm_type):
		"""Raise `ham_t.PermissionError`."""
		ham_t.flags.error_message = _("Insufficient Permission for {0}").format(self.doctype)
		raise ham_t.PermissionError
	...
	def insert(
		...
		if not (
			ham_t.flags.in_migrate or ham_t.local.flags.in_install or ham_t.flags.in_setup_wizard
		):
			if ham_t.get_cached_value("User", ham_t.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, ham_t.session.user)
		return self
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = ham_t.flags.in_test if ignore_version is None else ignore_version
	...
	def copy_attachments_from_amended_from(self):
		...
		from ham_t.desk.form.load import get_attachments
	...
	def update_child_table(self, fieldname, df=None):
		...
		if (
			...
			or ham_t.get_meta(df.options).is_virtual == 1
		):
		...
		if rows:
			...
			if len(deleted_rows) > 0:
				...
				ham_t.db.delete(df.options, {"name": ("in", tuple(row[0] for row in deleted_rows))})
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `ham_t.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		ham_t.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in ham_t.db.value_cache:
			del ham_t.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = ham_t.session.user
		...
		if self.is_new() and not (
			ham_t.flags.in_install or ham_t.flags.in_patch or ham_t.flags.in_migrate
		):
		...
		ham_t.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					ham_t.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					ham_t.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), ham_t.bold(_(df.label))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				ham_t.throw(msg, ham_t.NonNegativeError, title=_("Negative Value"))

	def validate_workflow(self):
		"""Validate if the workflow transition is valid"""
		if ham_t.flags.in_install == "ham_t":
			return
				...
				if fail:
					ham_t.throw(
						_("Value cannot be changed for {0}").format(
							ham_t.bold(self.meta.get_label(field.fieldname))
						),
						exc=ham_t.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		if ham_t.session.user == "Administrator":
			return
		...
		for table_field in self.meta.get_table_fields():
			all_fields += ham_t.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in ham_t.get_meta(table_field.options).fields or []:
				if df.permlevel and not df.permlevel in has_access_to:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or ham_t.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = ham_t.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = ham_t.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		if ham_t.flags.in_import:
			return

		new_doc = ham_t.new_doc(self.doctype, as_dict=True)
		self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = ham_t.new_doc(df.options, as_dict=True)
	...
	def check_if_latest(self):
		...
		if not self.get("__islocal") and not self.meta.get("is_virtual"):
			if self.meta.issingle:
				...
				if not tmp:
					ham_t.throw(_("Record does not exist"))
				else:
					tmp = tmp[0]
			...
			if conflict:
				ham_t.msgprint(
					...
					raise_exception=ham_t.TimestampMismatchError,
				)
		else:
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise ham_t.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise ham_t.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise ham_t.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if ham_t.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise ham_t.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			ham_t.throw(_("Could not find {0}").format(msg), ham_t.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			ham_t.throw(_("Cannot link cancelled document: {0}").format(msg), ham_t.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(ham_t.flags.in_import and ham_t.flags.mute_emails)
			or ham_t.flags.in_patch
		...
		from ham_t.email.doctype.notification.notification import evaluate_alert

		if self.flags.notifications is None:
			alerts = ham_t.cache().hget("notifications", self.doctype)
			if alerts is None:
				alerts = ham_t.get_all(
					"Notification",
					fields=["name", "event", "method"],
					filters={"enabled": 1, "document_type": self.doctype},
				)
				ham_t.cache().hset("notifications", self.doctype, alerts)
	...
	def _rename(
		...
		"""Rename the document. Triggers ham_t.rename_doc, then reloads."""
		from ham_t.model.rename_doc import rename_doc
	...
	def load_doc_before_save(self):
		...
		if not self.is_new():
			try:
				self._doc_before_save = ham_t.get_doc(self.doctype, self.name)
			except ham_t.DoesNotExistError:
				self._doc_before_save = None
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in ham_t.flags.currently_saving:
			ham_t.flags.currently_saving.remove((self.doctype, self.name))

		self.latest = None

	def clear_cache(self):
		ham_t.clear_document_cache(self.doctype, self.name)

	def reset_seen(self):
		"""Clear _seen property and set current user as seen"""
		if getattr(self.meta, "track_seen", False):
			ham_t.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([ham_t.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if ham_t.flags.in_patch:
			return

		ham_t.publish_realtime(
			"doc_update",
		...
		if (
			...
			data = {"doctype": self.doctype, "name": self.name, "user": ham_t.session.user}
			ham_t.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run ham_t.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in ham_t.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", ham_t.session.user)
	...
	def db_get(self, fieldname):
		...
		return ham_t.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from ham_t.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or ham_t.flags.in_install
			or (not self._doc_before_save and ham_t.flags.in_patch)
		):
			return

		version = ham_t.new_doc("Version")

		if is_useful_diff := version.update_version_info(self._doc_before_save, self):
			version.insert(ignore_permissions=True)

			if not ham_t.flags.in_migrate:
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not ham_t.compare(val1, condition, val2):
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			ham_t.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or ham_t.EmptyTableError
			)
		...
		out = ham_t.get_doc(
			{
				...
				"comment_email": comment_email or ham_t.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not ham_t.flags.read_only:
			_seen = self.get("_seen") or []
			_seen = ham_t.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				ham_t.db.set_value(self.doctype, self.name, "_seen", json.dumps(_seen), update_modified=False)
				ham_t.local.flags.commit = True
	...
	def add_viewed(self, user=None):
		...
		if hasattr(self.meta, "track_views") and self.meta.track_views:
			view_log = ham_t.get_doc(
				{
					"doctype": "View Log",
					"viewed_by": ham_t.session.user,
					"reference_doctype": self.doctype,
					"reference_name": self.name,
				}
			)
			if ham_t.flags.read_only:
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		if existing_key:
			doc = ham_t.get_doc("Document Share Key", existing_key)
		else:
			doc = ham_t.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", ham_t._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", ham_t._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		from ham_t.utils.background_jobs import enqueue
		...
		try:
			self.lock()
		except ham_t.DocumentLockedError:
			ham_t.throw(
		...
		return enqueue(
			"ham_t.model.document.execute_action",
			...
			if timeout:
				for i in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise ham_t.DocumentLockedError
		file_lock.create_lock(signature)
		ham_t.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in ham_t.local.locked_documents:
			ham_t.local.locked_documents.remove(self)
		...
		if date_diff(self.get(to_date_field), self.get(from_date_field)) < 0:
			ham_t.throw(
				_("{0} must be after {1}").format(
					ham_t.bold(self.meta.get_label(to_date_field)),
					ham_t.bold(self.meta.get_label(from_date_field)),
				),
				ham_t.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = ham_t.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from ham_t.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from ham_t.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = ham_t.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if ham_t.local.message_log:
			msg = json.loads(ham_t.local.message_log[-1]).get("message")
		else:
			msg = "<pre><code>" + ham_t.get_traceback() + "</pre></code>"
