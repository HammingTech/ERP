...
from hamming_t.query_builder import (
	get_qb_engine,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from hamming_t.utils.caching import request_cache
from hamming_t.utils.data import cstr, sbool
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from hamming_t.translate import get_full_dict
	from hamming_t.utils import is_html, strip_html_tags
...
def get_lang_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	from hamming_t.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `hamming_t.local.lang`"""
	from hamming_t.translate import get_user_lang
...
if TYPE_CHECKING:
	from hamming_t.database.mariadb.database import MariaDBDatabase
	from hamming_t.database.postgres.database import PostgresDatabase
	from hamming_t.model.document import Document
	from hamming_t.query_builder.builder import MariaDB, Postgres
	from hamming_t.utils.redis_wrapper import RedisWrapper
...
def init(site: str, sites_path: str = ".", new_site: bool = False) -> None:
	"""Initialize hamming_t for the current site. Reset thread locals `hamming_t.local`"""
...
def connect(
	...
	:param site: If site is given, calls `hamming_t.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from hamming_t.database import get_db
...
def get_traceback(with_context: bool = False) -> str:
	...
	from hamming_t.utils import get_traceback
...
def msgprint(
	...
	from hamming_t.utils import strip_html_tags
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `hamming_t.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def create_folder(path, with_init=False):
	...
	from hamming_t.utils import touch_file
...
def get_user():
	from hamming_t.utils.user import UserPermissions
...
def get_roles(username=None) -> list[str]:
	...
	if not local.session:
		return ["Guest"]
	import hamming_t.permissions

	return hamming_t.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from hamming_t.utils import md_to_html
	...
	from hamming_t.email.doctype.email_queue.email_queue import QueueBuilder
...
def is_whitelisted(method):
	from hamming_t.utils import sanitize_html
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `hamming_t.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import hamming_t.cache_manager
	import hamming_t.utils.caching

	if doctype:
		hamming_t.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		hamming_t.cache_manager.clear_user_cache(user)
	else:  # everything
		from hamming_t import translate

		hamming_t.cache_manager.clear_user_cache()
		hamming_t.cache_manager.clear_domain_cache()
	...
	hamming_t.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	permissions = hamming_t.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(
	...
	*,
	...
	Raises `hamming_t.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = hamming_t.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `hamming_t.PermissionError` if not permitted.
...
def get_precision(
	...
	from hamming_t.model.meta import get_field_precision
...
def new_doc(
	...
	from hamming_t.model.create_new import get_new_doc

	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)


def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `hamming_t.client.set_value`"""
	import hamming_t.client

	return hamming_t.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs) -> "Document":
	"""Return a `hamming_t.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = hamming_t.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = hamming_t.get_doc("ToDo", "TD0001")

	"""
	import hamming_t.model.document

	doc = hamming_t.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `hamming_t.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `hamming_t.model.meta.Meta` instance of given doctype name."""
	import hamming_t.model.meta

	return hamming_t.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import hamming_t.modules

	return hamming_t.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `hamming_t.model.delete_doc.delete_doc`.
	...
	import hamming_t.model.delete_doc

	return hamming_t.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return hamming_t.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `hamming_t.model.rename_doc.rename_doc`
	"""

	from hamming_t.model.rename_doc import rename_doc
...
def get_module_path(module, *joins):
	...
	from hamming_t.modules.utils import get_module_app
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "hamming_t" in apps:
		apps.remove("hamming_t")
	apps.insert(0, "hamming_t")

	return apps


@request_cache
def get_installed_apps(sort=False, hamming_t_last=False):
	...
	if hamming_t_last:
		if "hamming_t" in installed:
			installed.remove("hamming_t")
		installed.append("hamming_t")
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	import hamming_t.utils

	content = read_file(path, raise_not_found=raise_not_found)
	if content:
		content = hamming_t.utils.strip(content)
...
def import_doc(path):
	...
	from hamming_t.core.doctype.data_import.data_import import import_doc
...
def compare(val1, condition, val2):
	"""Compare two values using `hamming_t.utils.compare`
	...
	return hamming_t.utils.compare(val1, condition, val2)
...
def redirect(url):
	...
	from hamming_t.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        hamming_t.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import hamming_t.desk.reportview

	return hamming_t.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `hamming_t.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        hamming_t.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        hamming_t.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        hamming_t.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import hamming_t.model.db_query

	return hamming_t.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `hamming_t.model.db_query`. Will **not** check for permissions.
	Parameters are same as `hamming_t.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        hamming_t.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        hamming_t.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

	        # filter as a list of dicts
	        hamming_t.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `hamming_t.db.get_value`
...
def as_json(obj: dict | list, indent=1, separators=None) -> str:
	from hamming_t.utils.response import json_handler
...
def are_emails_muted():
	from hamming_t.utils import cint
...
def get_test_records(doctype):
	...
	from hamming_t.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import hamming_t.utils.formatters

	return hamming_t.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from hamming_t.utils.pdf import get_pdf
	from hamming_t.website.serve import get_response_content
...
def attach_print(
	...
	from hamming_t.utils import scrub_urls
...
def publish_progress(*args, **kwargs):
	...
	"""
	import hamming_t.realtime

	return hamming_t.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return hamming_t.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: hamming_t.local.cache[namespace]
	:param key: hamming_t.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import hamming_t.utils.background_jobs

	return hamming_t.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return hamming_t.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from hamming_t.utils.logger import get_logger
...
def get_active_domains():
	from hamming_t.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> hamming_t.get_version('User', 'foobar@gmail.com')
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from hamming_t.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from hamming_t.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from hamming_t.utils import squashify

	return squashify(results)


from hamming_t.desk.search import validate_and_sanitize_search_inputs  # noqa
