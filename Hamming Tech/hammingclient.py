"""
ham_tClient is a library that helps you connect with other ham_t systems
"""
import base64
import json
...
import ham_t
from ham_t.utils.data import cstr
...
class ham_tException(Exception):
	pass
...
class ham_tClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		ham_t_authorization_source=None,
	):
		self.headers = {
		...
		self.ham_t_authorization_source = ham_t_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.ham_t_authorization_source:
				auth_source = {"ham_t-Authorization-Source": self.ham_t_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": ham_t.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return ham_t._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "ham_t.client.insert_many", "docs": ham_t.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": ham_t.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return ham_t._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "ham_t.client.bulk_update", "docs": ham_t.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "ham_t.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "ham_t.client.submit", "doc": ham_t.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "ham_t.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": ham_t.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "ham_t.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(
		...
		for doc in docs:
			...
			if doctype != "User" and not ham_t.db.exists("User", doc.get("owner")):
				ham_t.get_doc(
	...
	def migrate_single(self, doctype):
		...
		doc.modified = ham_t.db.get_single_value(doctype, "modified")
		ham_t.get_doc(doc).insert()
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "ham_tClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise ham_tException(exc)
		if "message" in rjson:
...
class ham_tOAuth2Client(ham_tClient):
