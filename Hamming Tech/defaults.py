import ham_t
from ham_t.cache_manager import clear_defaults_cache, common_default_keys
...
def set_user_default(key, value, user=None, parenttype=None):
	set_default(key, value, user or ham_t.session.user, parenttype)


def add_user_default(key, value, user=None, parenttype=None):
	add_default(key, value, user or ham_t.session.user, parenttype)


def get_user_default(key, user=None):
	user_defaults = get_defaults(user or ham_t.session.user)
	...
	if is_a_user_permission_key(key):
		if d and isinstance(d, (list, tuple)) and len(d) == 1:
			...
			d = user_defaults.get(ham_t.scrub(key), None)
...
def is_a_user_permission_key(key):
	return ":" not in key and key != ham_t.scrub(key)


def not_in_user_permission(key, value, user=None):
	# returns true or false based on if value exist in user permission
	user = user or ham_t.session.user
	user_permission = get_user_permissions(user).get(ham_t.unscrub(key)) or []
...
def get_user_permissions(user=None):
	from ham_t.core.doctype.user_permission.user_permission import (
		get_user_permissions as _get_user_permissions,
	)

	"""Return ham_t.core.doctype.user_permissions.user_permissions._get_user_permissions (kept for backward compatibility)"""
...
def get_defaults(user=None):
	...
	if not user:
		user = ham_t.session.user if ham_t.session else "Guest"
...
def clear_user_default(key, user=None):
	clear_default(key, parent=user or ham_t.session.user)
...
def set_default(key, value, parent, parenttype="__default"):
	...
	if key_exists:
		ham_t.db.delete("DefaultValue", {"defkey": key, "parent": parent})
...
def clear_default(key=None, value=None, parent=None, name=None, parenttype=None):
	...
	ham_t.db.delete("DefaultValue", filters)
...
def get_defaults_for(parent="__default"):
	"""get all defaults"""
	defaults = ham_t.cache().hget("defaults", parent)
	...
	if defaults is None:
		...
		ham_t.cache().hset("defaults", parent, defaults)

	return defaults


def _clear_cache(parent):
	ham_t.clear_cache(user=parent if parent not in common_default_keys else None)
