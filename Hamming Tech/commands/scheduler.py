...
def trigger_scheduler_event(context, event):
	...
	for site in context.sites:
		try:
			...
			ham_t.connect()
			try:
				ham_t.get_doc("Scheduled Job Type", {"method": event}).execute()
...
def enable_scheduler(context):
	...
	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			ham_t.utils.scheduler.enable_scheduler()
			ham_t.db.commit()
...
def disable_scheduler(context):
	...
	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			ham_t.utils.scheduler.disable_scheduler()
			ham_t.db.commit()
...
def doctor(context, site=None):
	...
	from ham_t.utils.doctor import doctor as _doctor
...
def show_pending_jobs(context, site=None):
	...
	from ham_t.utils.doctor import pending_jobs as _pending_jobs
...
def purge_jobs(site=None, queue=None, event=None):
	...
	from ham_t.utils.doctor import purge_pending_jobs
...
def start_scheduler():
	from ham_t.utils.scheduler import start_scheduler
...
def start_worker(queue, quiet=False, rq_username=None, rq_password=None):
	...
	from ham_t.utils.background_jobs import start_worker
...
def ready_for_migration(context, site=None):
	from ham_t.utils.doctor import get_pending_jobs
