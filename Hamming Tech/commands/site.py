...
from ham_t.core.doctype.log_settings.log_settings import LOG_DOCTYPES
...
def new_site(
	...
	"Create a new site"
	from ham_t.installer import _new_site

	ham_t.init(site=site, new_site=True)
...
def restore(
	...
	from ham_t.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		if is_partial(decompressed_file_name):
			click.secho(
				"Partial Backup file detected. You cannot use a partial file to restore a ham_t site.",
				fg="red",
			)
			click.secho(
	...
	# dont allow downgrading to older versions of ham_t without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from ham_t.installer import extract_sql_from_archive, partial_restore
	from ham_t.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `bench restore` to restore a ham_t Site.", fg="red"
				)
				_backup.decryption_rollback()
				sys.exit(1)
...
def _reinstall(
	...
	try:
		...
		ham_t.connect()
		ham_t.clear_cache()
		installed = ham_t.get_installed_apps()
		ham_t.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps, force=False):
	...
	from ham_t.installer import install_app as _install_app
	...
	for site in context.sites:
		...
		for app in apps:
			...
			except ham_t.IncompatibleApp as err:
				err_msg = f":\n{err}" if str(err) else ""
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = ham_t.get_single("Installed Applications").installed_applications
		...
		if apps:
			...
			installed_applications = ham_t.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
	...
	if format == "json":
		click.echo(ham_t.as_json(summary_dict))
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		ham_t.connect(site=site)
		try:
			ham_t.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			ham_t.db.commit()
		finally:
			ham_t.destroy()
	if not context.sites:
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from ham_t.migrate import SiteMigration
...
@click.argument("ham_t_provider")
@pass_context
def migrate_to(context, ham_t_provider):
	"Migrates site to the specified provider"
	from ham_t.integrations.ham_t_providers import migrate_to

	for site in context.sites:
		ham_t.init(site=site)
		ham_t.connect()
		migrate_to(site, ham_t_provider)
		ham_t.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def run_patch(context, module, force):
	...
	import ham_t.modules.patch_handler

	for site in context.sites:
		ham_t.init(site=site)
		try:
			ham_t.connect()
			ham_t.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			ham_t.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			ham_t.reload_doc(module, doctype, docname, force=context.force)
			ham_t.db.commit()
		finally:
			ham_t.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			ham_t.reload_doctype(doctype, force=context.force)
			ham_t.db.commit()
		finally:
			ham_t.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		ham_t.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
...
def backup(
	...
	for site in context.sites:
		try:
			...
			exit_code = 1
			continue
		if ham_t.get_system_settings("encrypt_backup") and ham_t.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context, app):
	...
	from ham_t.installer import remove_from_installed_apps
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	from ham_t.installer import remove_app
...
def _drop_site(
	...
	from ham_t.database import drop_user_and_database
	from ham_t.utils.backups import scheduled_backup
	...
	drop_user_and_database(ham_t.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		ham_t.get_app_path("ham_t"), "..", "..", "..", "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	ham_t.init(site)
	old_path = ham_t.utils.get_site_path()
...
def set_user_password(site, user, password, logout_all_sessions=False):
	...
	from ham_t.utils.password import update_password
	...
	try:
		...
		ham_t.connect()
		if not ham_t.db.exists("User", user):
...
def set_last_active_for_user(context, user=None):
	...
	from ham_t.core.doctype.user.user import get_system_users
	...
	with ham_t.init_site(site):
		...
		ham_t.db.set_value("User", user, "last_active", now_datetime())
		ham_t.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from ham_t import publish_realtime
...
def browse(context, site, user=None):
	...
	from ham_t.auth import CookieManager, LoginManager
	...
	if site not in ham_t.utils.get_sites():
	...
	sid = ""
	if user:
		if ham_t.conf.developer_mode or user == "Administrator":
			ham_t.utils.set_request(path="/")
			ham_t.local.cookie_manager = CookieManager()
			ham_t.local.login_manager = LoginManager()
			ham_t.local.login_manager.login_as(user)
			sid = f"/app?sid={ham_t.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{ham_t.utils.get_site_url(site)}{sid}"
...
def start_recording(context):
	...
	for site in context.sites:
		ham_t.init(site=site)
		ham_t.set_user("Administrator")
		ham_t.recorder.start()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def start_ngrok(context, bind_tls):
	...
	port = ham_t.conf.http_port or ham_t.conf.webserver_port
...
def build_search_index(context):
	from ham_t.search.website_search import build_index_for_all_routes
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from ham_t.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from ham_t.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise ham_t.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context, dry_run, format, no_backup):
	...
	for site in context.sites:
		...
		queried_result = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == ham_t.conf.db_name)
			.run()
		)

		database_tables = [x[0] for x in queried_result]
		doctype_tables = ham_t.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			if format == "text":
				click.secho(f"No ghost tables found in {ham_t.local.site}...Great!", fg="green")
		else:
			...
			for table in TABLES_TO_DROP:
				...
				if not dry_run:
					ham_t.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[ham_t.local.site] = TABLES_TO_DROP
		ham_t.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		ham_t.conf.db_type,
		f"framework_{ham_t.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	from ham_t.model.meta import trim_tables
	from ham_t.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {ham_t.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {ham_t.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({ham_t.local.site: data}, indent=1, sort_keys=True))
	else:
