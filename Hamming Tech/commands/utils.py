...
from ham_t.coverage import CodeCoverage
from ham_t.exceptions import SiteNotSpecifiedError
from ham_t.utils import cint, update_progress_bar
...
def build(
	...
	from ham_t.build import bundle, download_ham_t_assets
	...
	if not (force or apps or os.environ.get("CI")):
		# skip building ham_t if assets exist remotely
		skip_ham_t = download_ham_t_assets(verbose=verbose)
	else:
		skip_ham_t = False

	# don't minify in developer_mode for faster builds
	development = ham_t.local.conf.developer_mode or ham_t.local.dev_server
	...
	bundle(mode, apps=apps, hard_link=hard_link, verbose=verbose, skip_ham_t=skip_ham_t)
...
def clear_cache(context):
	...
	from ham_t.desk.notifications import clear_notifications
...
def destroy_all_sessions(context, reason=None):
	...
	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			ham_t.sessions.clear_all_sessions(reason)
			ham_t.db.commit()
		finally:
			ham_t.destroy()
	if not context.sites:
...
def show_config(context, format):
	...
	from ham_t.utils.commands import render_table
	...
	for site in context.sites:
		...
		configuration = ham_t.get_site_config(sites_path=sites_path, site_path=site)
	...
	if format == "json":
		click.echo(ham_t.as_json(sites_config))
...
def reset_perms(context):
	...
	from ham_t.permissions import reset_perms

	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			for d in ham_t.db.sql_list(
				"""select name from `tabDocType`
				where istable=0 and custom=0"""
			):
				ham_t.clear_cache(doctype=d)
				reset_perms(d)
		finally:
			ham_t.destroy()
	if not context.sites:
...
def execute(context, method, args=None, kwargs=None, profile=False):
	...
	for site in context.sites:
		...
		try:
			...
			try:
				ret = ham_t.get_attr(method)(*args, **kwargs)
			except Exception:
				ret = ham_t.safe_eval(
			...
			ham_t.destroy()
		if ret:
			from ham_t.utils.response import json_handler
...
def export_doc(context, doctype, docname):
	...
	import ham_t.modules

	for site in context.sites:
		try:
			ham_t.init(site=site)
			ham_t.connect()
			ham_t.modules.export_doc(doctype, docname)
		finally:
			ham_t.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def export_json(context, doctype, path, name=None):
	...
	from ham_t.core.doctype.data_import.data_import import export_json
...
def export_csv(context, doctype, path):
	...
	from ham_t.core.doctype.data_import.data_import import export_csv
...
def export_fixtures(context, app=None):
	"Export fixtures"
	from ham_t.utils.fixtures import export_fixtures
...
def import_doc(context, path, force=False):
	...
	from ham_t.core.doctype.data_import.data_import import import_doc
...
def data_import(
	...
	from ham_t.core.doctype.data_import.data_import import import_file
...
def bulk_rename(context, doctype, path):
	...
	from ham_t.model.rename_doc import bulk_rename
	from ham_t.utils.csvutils import read_csv_content
...
def database(context):
	...
	if not site:
		raise SiteNotSpecifiedError
	ham_t.init(site=site)
	if not ham_t.conf.db_type or ham_t.conf.db_type == "mariadb":
		_mariadb()
...
def _mariadb():
	from ham_t.database.mariadb.database import MariaDBDatabase

	mysql = find_executable("mysql")
	command = [
		mysql,
		"--port",
		ham_t.conf.db_port or MariaDBDatabase.default_port,
		"-u",
		ham_t.conf.db_name,
		f"-p{ham_t.conf.db_password}",
		ham_t.conf.db_name,
		"-h",
		ham_t.conf.db_host or "localhost",
		"--pager=less -SFX",
		"--safe-updates",
		"-A",
	]
...
def _psql():
	...
	subprocess.run([psql, "-d", ham_t.conf.db_name])
...
def jupyter(context):
	...
	jupyter_notebooks_path = os.path.abspath(ham_t.get_site_path("jupyter_notebooks"))
	sites_path = os.path.abspath(ham_t.get_site_path(".."))
...
Run the following in your first cell to connect notebook to ham_t
```
import ham_t
ham_t.init(site='{site}', sites_path='{sites_path}')
ham_t.connect()
ham_t.local.lang = ham_t.db.get_default('lang')
ham_t.db.connect()
```
	""".format(
...
def console(context, autoreload=False):
	...
	all_apps = ham_t.get_installed_apps()
	failed_to_import = []
...
def transform_database(context, table, engine, row_format, failfast):
	...
	if ham_t.conf.db_type and ham_t.conf.db_type != "mariadb":
	...
	if table == "all":
		...
		queried_tables = (
			ham_t.qb.from_(information_schema.tables)
			.select("table_name")
			.where(
				(information_schema.tables.row_format != row_format)
				& (information_schema.tables.table_schema == ham_t.conf.db_name)
			)
			.run()
		)
	...
	for current, table in enumerate(tables):
		...
		try:
			ham_t.db.sql(f"ALTER TABLE `{table}`{values_to_set}")
...
def run_tests(
	...
	with CodeCoverage(coverage, app):
		import ham_t
		import ham_t.test_runner

		tests = test
		site = get_site(context)

		allow_tests = ham_t.get_conf(site).allow_tests
		...
		ham_t.flags.skip_before_tests = skip_before_tests
		ham_t.flags.skip_test_records = skip_test_records

		ret = ham_t.test_runner.main(
			app,
			module,
			doctype,
			module_def,
...
@click.option("--app", help="For App", default="ham_t")
...
def run_parallel_tests(
	...
	with CodeCoverage(with_coverage, app):
		site = get_site(context)
		if use_orchestrator:
			from ham_t.parallel_test_runner import ParallelTestWithOrchestrator

			ParallelTestWithOrchestrator(app, site=site)
		else:
			from ham_t.parallel_test_runner import ParallelTestRunner
...
def run_ui_tests(
	...
	"Run UI tests"
	site = get_site(context)
	app_base_path = os.path.abspath(os.path.join(ham_t.get_app_path(app), ".."))
	site_url = ham_t.utils.get_site_url(site)
	admin_password = ham_t.get_conf(site).admin_password
	...
	if not (
		...
		ham_t.commands.popen(f"yarn add {packages} --no-lockfile")
	...
	ham_t.commands.popen(formatted_command, cwd=app_base_path, raise_err=True)
...
def serve(
	...
	with CodeCoverage(with_coverage, "ham_t"):
		if with_coverage:
...
def request(context, args=None, path=None):
	...
	for site in context.sites:
		try:
			...
			if args:
				if "?" in args:
					ham_t.local.form_dict = ham_t._dict([a.split("=") for a in args.split("?")[-1].split("&")])
				else:
					ham_t.local.form_dict = ham_t._dict()

				if args.startswith("/api/method"):
					ham_t.local.form_dict.cmd = args.split("?")[0].split("/")[-1]
			elif path:
			...
			ham_t.handler.execute_cmd(ham_t.form_dict.cmd)

			print(ham_t.response)
		finally:
			ham_t.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def make_app(destination, app_name, no_git=False):
	...
	from ham_t.utils.boilerplate import make_boilerplate
...
def set_config(context, key, value, global_=False, parse=False, as_dict=False):
	...
	from ham_t.installer import update_site_config

	if as_dict:
		from ham_t.utils.commands import warn
...
def get_version(output):
	...
	from ham_t.utils.change_log import get_app_branch
	from ham_t.utils.commands import render_table

	ham_t.init("")
	data = []

	for app in sorted(ham_t.get_all_apps()):
		module = ham_t.get_module(app)
		app_hooks = ham_t.get_module(app + ".hooks")

		app_info = ham_t._dict()

		try:
			app_info.commit = Repo(ham_t.get_app_path(app, "..")).head.object.hexsha[:7]
...
def rebuild_global_search(context, static_pages=False):
	...
	from ham_t.utils.global_search import (
