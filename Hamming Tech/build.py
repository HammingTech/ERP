...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "ham_t", "dist", type)
	...
	assets_json = ham_t.read_file("assets/assets.json")
	if assets_json:
		assets_json = ham_t.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/ham_t"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["ham_t/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="ham_t")


def get_assets_link(ham_t_head) -> str:
	tag = getoutput(
		r"cd ../apps/ham_t && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % ham_t_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/ham_t/ham_t/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.ham_tframework.com/{ham_t_head}.tar.gz"

	if not head(url):
		reference = f"Release {tag}" if tag else f"Commit {ham_t_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, ham_t_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="ham_t-assets-", suffix=ham_t_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded ham_t assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./ham_t-bench/sites", "")
...
def download_ham_t_assets(verbose=True):
	"""Downloads and sets up ham_t assets if they exist based on the current
	commit HEAD.
	Returns True if correctly setup else returns False.
	"""
	ham_t_head = getoutput("cd ../apps/ham_t && git rev-parse HEAD")

	if not ham_t_head:
		return False

	try:
		url = get_assets_link(ham_t_head)
		assets_archive = fetch_assets(url, ham_t_head)
...
def setup():
	...
	pymodules = []
	for app in ham_t.get_all_apps(True):
		try:
			pymodules.append(ham_t.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(ham_t.local.sites_path, "assets")
...
def bundle(
	...
	ham_t_app_path = ham_t.get_app_path("ham_t", "..")
	ham_t.commands.popen(command, cwd=ham_t_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = ham_t.utils.cint(os.environ.get("LIVE_RELOAD", ham_t.conf.live_reload))
	...
	check_node_executable()
	ham_t_app_path = ham_t.get_app_path("ham_t", "..")
	ham_t.commands.popen(command, cwd=ham_t_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in ham_t.get_all_apps():
		app_doc_path = None

		pymodule = ham_t.get_module(app_name)
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `ham_t.templates`"""
	return """ham_t.templates["{key}"] = '{content}';\n""".format(
