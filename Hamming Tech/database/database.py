...
import ham_t.model.meta
from ham_t import _
from ham_t.database.utils import (
	EmptyQueryValues,
	FallBackDateTimeStr,
	LazyMogrify,
	Query,
...
from ham_t.exceptions import DoesNotExistError, ImplicitCommitError
from ham_t.model.utils.link_count import flush_local_link_count
from ham_t.query_builder.functions import Count
from ham_t.query_builder.utils import DocType
from ham_t.utils import cast as cast_fieldtype
from ham_t.utils import get_datetime, get_table_name, getdate, now, sbool
...
class Database:
	...
	class InvalidColumnName(ham_t.ValidationError):
		pass
	...
	def __init__(
		...
		self.setup_type_map()
		self.host = host or ham_t.conf.db_host or "127.0.0.1"
		self.port = port or ham_t.conf.db_port or ""
		self.user = user or ham_t.conf.db_name
		self.db_name = ham_t.conf.db_name
		self._conn = None

		if ac_name:
			self.user = ac_name or ham_t.conf.db_name

		if use_default:
			self.user = ham_t.conf.db_name

		self.transaction_writes = 0
		self.auto_commit_on_many_writes = 0

		self.password = password or ham_t.conf.db_password
		self.value_cache = {}
		# self.db_type: str
	...
	def connect(self):
		...
		ham_t.local.rollback_observers = []
	...
	def sql(
		...
		Examples:
		        ...
		        ham_t.db.sql("select name from tabCustomer", as_dict=True)

		        # return names beginning with a
		        ham_t.db.sql("select name from tabCustomer where name like %s", "a%")

		        # values as dict
		        ham_t.db.sql("select name from tabCustomer where name like %(name)s and owner=%(owner)s",
		                {"name": "a%", "owner":"test@example.com"})

		"""
		if isinstance(query, (MySQLQueryBuilder, PostgreSQLQueryBuilder)):
			ham_t.errprint("Use run method to execute SQL queries generated by Query Engine")
		...
		try:
			...
			if self.is_syntax_error(e):
				ham_t.errprint(f"Syntax error in query:\n{query} {values}")

			elif self.is_deadlocked(e):
				raise ham_t.QueryDeadlockError(e) from e

			elif self.is_timedout(e):
				raise ham_t.QueryTimeoutError(e) from e
				...
				ham_t.throw(
					...
					exc=ham_t.InReadOnlyMode,
				)

			# TODO: added temporarily
			elif self.db_type == "postgres":
				traceback.print_stack()
				ham_t.errprint(f"Error in query:\n{e}")
				raise

			elif isinstance(e, self.ProgrammingError):
				if ham_t.conf.developer_mode:
					traceback.print_stack()
					ham_t.errprint(f"Error in query:\n{query, values}")
				raise
		...
		if debug:
			time_end = time()
			ham_t.errprint(f"Execution time: {time_end - time_start:.2f} sec")
	...
	def _log_query(self, mogrified_query: str, debug: bool = False, explain: bool = False) -> None:
		...
		if ham_t.conf.allow_tests and ham_t.cache().get_value("flag_print_sql"):
		...
		if debug:
			...
			ham_t.errprint(_query)

		if ham_t.conf.logging == 2:
			_query = _query or str(mogrified_query)
			ham_t.log(f"<<<< query\n{_query}\n>>>>")

		if ham_t.flags.in_migrate:
			_query = _query or str(mogrified_query)
	...
	def mogrify(self, query: Query, values: QueryValues):
		...
		try:
			...
			if isinstance(values, dict):
				return query % {k: ham_t.db.escape(v) if isinstance(v, str) else v for k, v in values.items()}
			elif isinstance(values, (list, tuple)):
				return query % tuple(ham_t.db.escape(v) if isinstance(v, str) else v for v in values)
			return query, values
	...
	def explain_query(self, query, values=None):
		...
		ham_t.errprint("--- query explain ---")
		try:
			self._cursor.execute(f"EXPLAIN {query}", values)
		except Exception as e:
			ham_t.errprint(f"error in query explain: {e}")
		else:
			ham_t.errprint(json.dumps(self.fetch_as_dict(), indent=1))
			ham_t.errprint("--- query explain end ---")
	...
	def sql_list(self, query, values=(), debug=False, **kwargs):
		...
		Example:
		        ...
		        doctypes = ham_t.db.sql_list("select name from DocType")
		"""
		...
		if query[:6].lower() in ("update", "insert", "delete"):
			...
			if self.transaction_writes > self.MAX_WRITES_PER_TRANSACTION:
				if self.auto_commit_on_many_writes:
					...
					raise ham_t.TooManyWritesError(msg)
	...
	def fetch_as_dict(self, formatted=0, as_utf8=0) -> list[ham_t._dict]:
		...
		for r in result:
			...
			ret.append(ham_t._dict(zip(keys, values)))
		return ret

	@staticmethod
	def clear_db_table_cache(query):
		if query and is_query_type(query, ("drop", "create")):
			ham_t.cache().delete_key("db_tables")
	...
	def get_value(
		...
		*,
		...
		Example:
		        ...
		        ham_t.db.get_value("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        ham_t.db.get_value("User", "test@example.com", "last_login")

		        last_login, last_ip = ham_t.db.get_value("User", "test@example.com",
		                ["last_login", "last_ip"])

		        # returns default date_format
		        ham_t.db.get_value("System Settings", None, "date_format")
		"""
	...
	def get_values(
		...
		*,
		...
		Example:
		        ...
		        # return first customer starting with a
		        customers = ham_t.db.get_values("Customer", {"name": ("like a%")})

		        # return last login of **User** `test@example.com`
		        user = ham_t.db.get_values("User", "test@example.com", "*")[0]
		"""
		out = None
		...
		if isinstance(filters, list):
			...
			if (filters is not None) and (filters != doctype or doctype == "DocType"):
				try:
					...
					if ignore and (ham_t.db.is_missing_column(e) or ham_t.db.is_table_missing(e)):
						# table or column not found, return None
						out = None
					elif (not ignore) and ham_t.db.is_table_missing(e):
	...
	def get_singles_dict(self, doctype, debug=False, *, for_update=False, cast=False):
		...
		Example:
		        ...
		        account_settings = ham_t.db.get_singles_dict("Accounts Settings")
		"""
		queried_result = ham_t.qb.engine.get_query(
			"Singles",
		...
		if not cast:
			return ham_t._dict(queried_result)

		try:
			meta = ham_t.get_meta(doctype)
		except DoesNotExistError:
			return ham_t._dict(queried_result)

		return_value = ham_t._dict()
	...
	def get_all(*args, **kwargs):
		return ham_t.get_all(*args, **kwargs)

	@staticmethod
	def get_list(*args, **kwargs):
		return ham_t.get_list(*args, **kwargs)
	...
	def set_single_value(
		...
		**kwargs,
		...
		Example:
		        ...
		        company = ham_t.db.set_single_value("System Settings", "deny_multiple_sessions", True)
		"""
	...
	def get_single_value(self, doctype, fieldname, cache=True):
		...
		Example:
		        ...
		        company = ham_t.db.get_single_value('Global Defaults', 'default_company')
		"""
		...
		df = ham_t.get_meta(doctype).get_field(fieldname)

		if not df:
			ham_t.throw(
				_("Invalid field name: {0}").format(ham_t.bold(fieldname)), self.InvalidColumnName
			)
	...
	def _get_values_from_table(
		...
		*,
		...
		field_objects = []
		query = ham_t.qb.engine.get_query(
			table=doctype,
			filters=filters,
			orderby=order_by,
			for_update=for_update,
	...
	def _get_value_for_many_names(
		...
		*,
		...
		if names := list(filter(None, names)):
			return ham_t.qb.engine.get_query(
				doctype,
				fields=field,
				filters=names,
				order_by=order_by,
	...
	def set_value(
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if update_modified:
			...
			modified_by = modified_by or ham_t.session.user
		...
		if is_single_doctype:
			...
			query = (
				ham_t.qb.into("Singles").columns("doctype", "field", "value").insert(*singles_data)
			).run(debug=debug)
			ham_t.clear_document_cache(dt, dt)
			...
			if for_update:
				...
				query = ham_t.qb.update(table).where(table.name.isin(docnames))

				for docname in docnames:
					ham_t.clear_document_cache(dt, docname)

			else:
				query = ham_t.qb.engine.build_conditions(table=dt, filters=dn, update=True)
				# TODO: Fix this; doesn't work rn - gavin@ham_t.io
				# ham_t.cache().hdel_keys(dt, "document_cache")
				# Workaround: clear all document caches
				ham_t.cache().delete_value("document_cache")
	...
	def touch(self, doctype, docname):
		...
		modified = now()
		DocType = ham_t.qb.DocType(doctype)
		ham_t.qb.update(DocType).set(DocType.modified, modified).where(DocType.name == docname).run()
		return modified

	@staticmethod
	def set_temp(value):
		"""Set a temperory value and return a key."""
		key = ham_t.generate_hash()
		ham_t.cache().hset("temp", key, value)
		return key

	@staticmethod
	def get_temp(key):
		"""Return the temperory value and delete it."""
		return ham_t.cache().hget("temp", key)
	...
	def set_default(key, val, parent="__default", parenttype=None):
		...
		ham_t.defaults.set_default(key, val, parent, parenttype)
	...
	def add_default(key, val, parent="__default", parenttype=None):
		...
		ham_t.defaults.add_default(key, val, parent, parenttype)

	@staticmethod
	def get_defaults(key=None, parent="__default"):
		"""Get all defaults"""
		defaults = ham_t.defaults.get_defaults_for(parent)
		if not key:
			return defaults
		...
		return defaults.get(ham_t.scrub(key))

	def begin(self, *, read_only=False):
		read_only = read_only or ham_t.flags.read_only
		mode = "READ ONLY" if read_only else ""
		self.sql(f"START TRANSACTION {mode}")

	def commit(self):
		"""Commit current transaction. Calls SQL `COMMIT`."""
		for method in ham_t.local.before_commit:
			ham_t.call(method[0], *(method[1] or []), **(method[2] or {}))
	...
	def add_before_commit(self, method, args=None, kwargs=None):
		ham_t.local.before_commit.append([method, args, kwargs])

	@staticmethod
	def flush_realtime_log():
		for args in ham_t.local.realtime_log:
			ham_t.realtime.emit_via_redis(*args)

		ham_t.local.realtime_log = []

	def savepoint(self, save_point):
		"""Savepoints work as a nested transaction.

		Changes can be undone to a save point by doing ham_t.db.rollback(save_point)
	...
	def rollback(self, *, save_point=None):
		...
		if save_point:
			...
			self.sql("rollback")
			self.begin()
			for obj in dict.fromkeys(ham_t.local.rollback_observers):
	...
	def a_row_exists(self, doctype):
		...
		return ham_t.get_all(doctype, limit=1, order_by=None, as_list=True)
	...
	def count(self, dt, filters=None, debug=False, cache=False, distinct: bool = True):
		...
		if cache and not filters:
			cache_count = ham_t.cache().get_value(f"doctype:count:{dt}")
			if cache_count is not None:
				return cache_count
		count = ham_t.qb.engine.get_query(
			table=dt, filters=filters, fields=Count("*"), distinct=distinct
		).run(debug=debug)[0][0]
		if not filters and cache:
			ham_t.cache().set_value(f"doctype:count:{dt}", count, expires_in_sec=86400)
		return count
	...
	def get_creation_count(self, doctype, minutes):
		...
		Table = ham_t.qb.DocType(doctype)

		return (
			ham_t.qb.from_(Table)
	...
	def get_db_table_columns(self, table) -> list[str]:
		...
		columns = ham_t.cache().hget("table_columns", table)
		if columns is None:
			information_schema = ham_t.qb.Schema("information_schema")

			columns = (
				ham_t.qb.from_(information_schema.columns)
			...
			if columns:
				ham_t.cache().hset("table_columns", table, columns)
	...
	def get_system_setting(self, key):
		return ham_t.get_system_settings(key)
	...
	def is_column_missing(e):
		return ham_t.db.is_missing_column(e)

	def get_descendants(self, doctype, name):
		"""Return descendants of the group node in tree"""
		from ham_t.utils.nestedset import get_descendants_of
	...
	def delete(self, doctype: str, filters: dict | list = None, debug=False, **kwargs):
		...
		query = ham_t.qb.engine.build_conditions(table=doctype, filters=filters).delete()
	...
	def log_touched_tables(self, query):
		if is_query_type(query, ("insert", "delete", "update", "alter", "drop", "rename")):
			...
			if ham_t.flags.touched_tables is None:
				ham_t.flags.touched_tables = set()
			ham_t.flags.touched_tables.update(tables)
	...
	def create_sequence(self, *args, **kwargs):
		from ham_t.database.sequence import create_sequence

		return create_sequence(*args, **kwargs)

	def set_next_sequence_val(self, *args, **kwargs):
		from ham_t.database.sequence import set_next_val

		set_next_val(*args, **kwargs)

	def get_next_sequence_val(self, *args, **kwargs):
		from ham_t.database.sequence import get_next_val

		return get_next_val(*args, **kwargs)


def enqueue_jobs_after_commit():
	from ham_t.utils.background_jobs import (
		RQ_JOB_FAILURE_TTL,
		RQ_RESULTS_TTL,
		execute_job,
		get_queue,
	...
	if ham_t.flags.enqueue_after_commit and len(ham_t.flags.enqueue_after_commit) > 0:
		for job in ham_t.flags.enqueue_after_commit:
		...
		ham_t.flags.enqueue_after_commit = []
...
def savepoint(catch: type | tuple[type, ...] = Exception):
	...
	try:
		...
		ham_t.db.savepoint(savepoint)
		yield  # control back to calling function
	except catch:
		ham_t.db.rollback(save_point=savepoint)
	else:
		ham_t.db.release_savepoint(savepoint)
